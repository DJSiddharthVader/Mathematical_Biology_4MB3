...
\end{equation}
 The EE is $(\frac{\mu + \gamma}{\beta}, \frac{\mu}{\beta}(R_0 - 1))$ so the characteristic polynomial at the EE is:
 
 \begin{align*}
  0 &= (-\beta \hat{I} - \mu - \lambda)(\beta \hat{S} - (\mu + \gamma) - \lambda) + \beta^2 \hat{S}\hat{I}\\
  0 &= (-\mu(R_0 - 1) - \mu - \lambda)(- \lambda) + (\mu + \gamma) \mu(R_0 - 1)\\
  0 &= \lambda^2 + (\mu R_0)\lambda + (\mu + \gamma)\mu(R_0 - 1)\\
\end{align*}

Solving for the eigenvalue, $\lambda$, using the quadratic equation:
\begin{equation}
  \lambda = -\frac{\mu R_0}{2} \pm \frac {\sqrt{(\mu R_0)^2-4(\mu + \gamma)\mu(R_0 - 1)}}{2}
\end{equation}

Assuming that$(\mu R_0)^2$ is relatively small, then this eigenvalue will be complex, $\lambda = -\frac{\mu R_0}{2} \pm iM$. The period of damped oscillations, $T$, is $T = 2\pi\frac{1}{M}$ and the $e$-folding time, $t_e = \frac{1}{re(\lambda)}$, is $t_e = \frac{2}{\mu R_0}$. \\

Furthermore if we assume that $(\mu R_0)^2 \approx 0$, the above can be simplified to:

\begin{equation}
  \lambda \approx -\frac{\mu R_0}{2} \pm i \sqrt{(\mu + \gamma)\mu(R_0 - 1)}
\end{equation}

So the period of damped oscillations can be approximated as $T \approx \frac{2\pi}{\sqrt{(\mu + \gamma)\mu(R_0 - 1)}}$. \par
