...
<<<multiPatch,fig.height=6,echo=FALSE,fig.cap=mpcap>>=
    ...
    ic <- list(rows=rows,
               ...
               i0=i0,
               w0=w0)
    params <- list(patches=rows*cols, mu=mu, beta_i = beta_i, gamma = gamma, sigma = sigma, beta_w = beta_w)
    tmax <- 500
    steps <- 5000
