...
<<mpcaption,echo=FALSE>>=
    dec <- 5
    mpcap <- paste('\\label{fig:multitreatment} Plot comparing treatments for the multipatch SIRW model',
                   '$\\mu=',round(mu,dec),
                   ',\\beta_i=',round(beta_i,dec),
                   ',\\gamma=',round(gamma,dec),
                   ',\\sigma=',round(sigma,dec),
                   ',\\beta_w=',round(beta_w,dec),
                   ',\\xi=',round(xi,dec),
                   ',\\alpha=',alpha,
                   ...
                   sep='')
@
<<compare,fig.height=10,fig.width=8,echo=FALSE,fig.cap=mpcap>>=
    source('spatialSIR.R')
    #multipatch
    tmax <- 100
    steps <- 200
...
@
\FloatBarrier
<<mphlcaption,echo=FALSE>>=
    dec <- 5
    mpcap <- paste('\\label{fig:multipatch} Plot of different treatments for the multipatch model with high and low severity infectives.',
                   '$\\mu=',round(mu,dec),
                   ',\\beta_l=',round(beta_l,dec),
                   ',\\beta_h=',round(beta_h,dec),
                   ',\\gamma=',round(gamma,dec),
                   ',\\sigma=',round(sigma,dec),
                   ',\\beta_w=',round(beta_w,dec),
                   ',\\xi_l=',round(xi_l,dec),
                   ...
                   sep='')
@
<<compareHL,fig.height=10,fig.width=8,echo=FALSE,fig.cap=mpcap>>=
    source('spatialSIR.R')
    #multipatch
    tmax <- 150
    steps <- 200
