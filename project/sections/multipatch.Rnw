...
<<mpcaption,echo=FALSE>>=
    ...
    s0 <- 1-i0
    w0 <- 0.13
    mpcap <- paste('\\label{fig:multipatch} Plot of the SIRW model for all patches in a multi patch model. Parameters are ',
                   '$\\mu=',round(mu,2),
                   ...
                   sep='')
@
<<<multiPatch,fig.show='animate',fig.height=6,echo=FALSE,fig.cap=mpcap>>=
    source('spatialSIR.R')
    #multipatch
    rows <- 10
    cols <- rows
    ...
    tmax <- 500
    steps <- 50
    soln <- solveMultiModel(multiWaterModel,ic,params, tmax=tmax,steps=steps)
    image(soln, select=1:4, axes=FALSE, legend = TRUE, ask=FALSE, main=c("Susceptible", "Infectious", "Recovered","Water"), mfrow=c(2,2))
@
\FloatBarrier
